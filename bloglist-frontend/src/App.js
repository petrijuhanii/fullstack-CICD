...
const App = () => {
  ...
  const likeBlog = async (id) => {
    ...
    blogService.update(liked.id, liked).then(updatedBlog => {
      setSuccesMessage(`you liked '${updatedBlog.title}' by ${updatedBlog.author}`)
      const updatedBlogs = blogs
        .map(b => b.id===id ? updatedBlog : b)
        .sort((b1, b2) => b2.likes>b1.likes ? 1 : -1)
    ...
    return (
      <div>
        ...
        {blogs.sort((a, b) => b.likes - a.likes).map(blog =>
          <Blog key={blog.id} blog={blog} username={user.username} addLike={likeBlog}/>
        )}
